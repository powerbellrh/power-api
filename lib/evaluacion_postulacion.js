...
function construirNotaTeamtailor(resultadoEvaluacion, tituloVacante, esReevaluacion) {
  const marcador = '#PREGUNTAS#';
  const primero  = resultadoEvaluacion.indexOf(marcador);
  ...
  const encabezado = `${esReevaluacion ? 'Segunda' : 'Primera'} evaluación: ${tituloVacante}`;
  return `${encabezado}\n\n${cuerpo.trim()}`;
}
