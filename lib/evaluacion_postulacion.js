...
function extraerPrimerasNuevePreguntas(resultadoEvaluacion) {
  return extraerPreguntas(resultadoEvaluacion, 9);
}
...
function obtenerUrlImagenPuntuacion(calificacion, verificado = false) {
  if (calificacion == null) return null;
  const base = 'https://nihtmbtirvhgumfbavvc.supabase.co/storage/v1/object/public/iconos/';
  const sufijo = verificado ? '%20-%20Verificado.png' : '.png';
  if (calificacion >= 18) return base + 'Ideal' + sufijo;
  if (calificacion >= 15) return base + 'Altamente%20compatible' + sufijo;
  if (calificacion >= 12) return base + 'Compatible' + sufijo;
  if (calificacion >= 9)  return base + 'Parcialmente%20compatible' + sufijo;
  return base + 'No%20compatible' + sufijo;
}
...
function construirBloqueInfoCandidato(nombre, respuestas) {
...
function pareceNumeroTelefono(nombre) {
...
function construirNotaTeamtailor(resultadoEvaluacion, tituloVacante) {
  const marcador = '#PREGUNTAS#';
  const primero  = resultadoEvaluacion.indexOf(marcador);
  const segundo  = primero === -1 ? -1 : resultadoEvaluacion.indexOf(marcador, primero + marcador.length);

  const cuerpo = segundo === -1
    ? resultadoEvaluacion
    : resultadoEvaluacion.slice(0, primero).trimEnd()
      + '\n\n❓ Preguntas\n'
      + resultadoEvaluacion.slice(primero + marcador.length, segundo).trim();

  const encabezado = `Primera evaluación: ${tituloVacante}`;
  return `${encabezado}\n\n${cuerpo.trim()}`;
}
