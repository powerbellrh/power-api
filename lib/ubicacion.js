...
export async function verificarProximidadUbicacion(vacanteId, direccion) {
  ...
  try {
    ...
    const { lat, lng } = ubicacion.punto_coordenadas;
    const origenes    = `${lat},${lng}`;
    ...
    if (datosCaminando.status !== 'OK' || datosManejando.status !== 'OK')
      return { encontrado: false, razon: 'error_api_google' };

    const elCaminando = datosCaminando.rows[0].elements[0];
    const elManejando = datosManejando.rows[0].elements[0];

    if (elCaminando.status !== 'OK' || elManejando.status !== 'OK')
      return { encontrado: false, razon: 'ruta_no_encontrada' };

    return {
      encontrado:              true,
      nombre_ubicacion:        ubicacion.punto_nombre        ?? 'Sin nombre',
      descripcion_ubicacion:   ubicacion.punto_descripcion   ?? '',
