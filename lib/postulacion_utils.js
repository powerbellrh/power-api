...
function extractQuestions(evaluationResult, count) {
  ...
  if (questions.length < count) throw new Error(`Insufficient questions: ${questions.length}/${count}`);
  return questions.slice(0, count);
}

function extractFirstFiveQuestions(evaluationResult) {
  return extractQuestions(evaluationResult, 5);
}

function extractFirstThreeQuestions(evaluationResult) {
  return extractQuestions(evaluationResult, 3);
}
...
function extractEvaluationStatus(text) {
  if (/#(?:✅\s*)?APTO#/i.test(text) && !/#(?:❌\s*)?NO\s+APTO#/i.test(text)) return 'APTO';
  if (/#(?:❌\s*)?NO\s+APTO#/i.test(text)) return 'NO APTO';
  return null;
}

function evaluationStatusToScore(status) {
  if (status === 'APTO')    return 20;
  if (status === 'NO APTO') return 0;
  return null;
}

function getEvaluationStatusRating(status) {
  if (status === 'APTO')    return 5;
  if (status === 'NO APTO') return 1;
  return null;
}
...
function isImageUrl(url) {
  ...
  return ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp'].some(ext => lower.includes(ext))
    || lower.includes('image') || lower.includes('imgur') || lower.includes('cloudinary');
}

function extractImageUrlFromAnswers(answers) {
  for (const answer of answers) {
    const attrs = answer.attributes;
    const qt    = attrs['question-type']?.toLowerCase();
    if (qt !== 'text') continue;
    const value = attrs.text || attrs.answer || '';
    if (value && isImageUrl(value)) return value.trim();
  }
  return null;
}
