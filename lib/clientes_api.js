...
async function mcGet(path, params = {}) {
  const qs = new URLSearchParams(params).toString();
  const url = `https://api.manychat.com${path}${qs ? `?${qs}` : ''}`;
  const r = await fetch(url, { headers: mcHeaders() });
  if (!r.ok) throw new Error(`ManyChat GET ${path} → ${r.status}: ${await r.text()}`);
  return r.json();
}

export { ttGet, ttPatch, ttPost, mcPost, mcGet };
